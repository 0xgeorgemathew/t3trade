...
export const TradingMeasureVolatilityTool = Tool.make("trading_measure_volatility", {
  description:
    ...
    "Four more fields describe the window as a RANGE, so a range read costs nothing to make and can be checked against a number rather than argued: `swingHighUsd` and `swingLowUsd` are the measured boundaries (`swingRangeUsd` is their difference), `positionInRangePercent` is where the reference price sits between them (0 on the floor, 100 on the ceiling, 50 mid-range, and 50 when the window has no height), and `excursionSymmetryRatio` is the median favourable-up move divided by the median favourable-down move at the LONGEST published horizon. Near 1 the window has been paying longs and shorts alike, which is what ranging looks like; far from 1 one side has been paying and the window is trending. It is 0 when there is no horizon to read from. " +
...
export const TradingGetTradeHistoryTool = Tool.make("trading_get_trade_history", {
  description:
    ...
    "`roundTrips` is the same trading paired into what it actually DID: one entry flat to flat, with direction, size, size-weighted entry and exit price, hold duration, gross PnL, the fees BOTH sides paid, and the net. An order list cannot tell you what a trade was worth — the entry and the exit are separate rows in it — so this is the granularity a scalp is judged at. A position still open has no trip here, because it has no result yet. " +
    "`summary.recentFeeShareOfGrossPercent` is fees as a percent of the gross the last three trips produced, and it is the one number that says the size is wrong for the range: above 50, the trades are working and the costs are taking the result. `feeShareOfGrossPercent` is the same reading over the mission's whole life, which is the one that keeps looking fine while the recent one deteriorates. Both take gross as a magnitude, so a loss and a win do not cancel and leave the fees looking free. " +
