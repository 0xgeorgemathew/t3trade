...
export const TradingRegisterWatchTool = Tool.make("trading_register_watch", {
  description:
    ...
    '`pnl_above` fires when the reconciled unrealised PnL for `market` is at least `valueUsd`; a flat position never fires it. While you hold a position, the runtime also arms its own `pnl_above` at the strategy\'s `protection.targetProfitUsd` with `armedReason: "profit_target"` — that is the wake-and-decide win target, and a wake from it is the cue to bank the profit or republish with a higher target. ' +
