...
*is the expected move over your intended hold bigger than the round trip is
worth?* No taxonomy, no tournament.
...
**Risk to watch:** adverse selection on maker *entries*. Track fill rate and
...
a new reason: *a plan exists and its triggers are armed.* Collapse the nine-value
...
### Step 6.2 — `plan(...)`, `enter(...)`, `exit(...)`
Retire the quote-then-execute two-step and its tables (`trading_entry_quotes`,
`trading_execution_sequences`). **`enter` must work with or without a plan.**
...
the model's job changes from *decide* to *execute faithfully and report
deviations*.
...
question *do EMA-cross entries actually pay?* is answered from real trades.
