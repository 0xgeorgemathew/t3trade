...
export const PLAYBOOKS: ReadonlyArray<Playbook> = [
  ...
  },
  {
    ...
    procedure: [
      ...
      "Target 60-70% of the range height, not the whole crossing — the boundary rarely gets touched and you are not there to pick the last dollar. Size `targetProfitUsd` off that DISCOUNTED capture, name the full range height in the rationale, and publish `takeProfitPrice` at the boundary the capture reaches; a target claiming the whole crossing is claiming a move you are not trying to take.",
  ...
  },
  {
    ...
    procedure: [
      ...
      "Show the derivation in the rationale — the measurement, the lookback, the holding period — and set `targetProfitUsd` to the CONSERVATIVE rung, the one you would genuinely bank. Put the whole ladder — conservative, base, extension — in `protection.targetProfitRationale`, and publish `takeProfitPrice` when the plan names the level it is aiming at; that is the price the server rests the take-profit at.",
  ...
  },
  {
    ...
    procedure: [
      ...
      "Stop at the opposite edge of the opening range; target one range height in the break direction, sized off that height in `targetProfitUsd` with `takeProfitPrice` at the level it reaches.",
    ],
    gates: [
  ...
  },
  {
    ...
    procedure: [
      ...
      "Play it for " +
        ...
        "% of the swing range — the move back toward the middle, not a reversal. Size `targetProfitUsd` off that partial capture, and say in the rationale that the full height is not what is being claimed.",
